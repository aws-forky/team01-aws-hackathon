...
async def evaluate_all_answers(request: AllEvaluateRequest):
    try:
        feedback = await llm_processor.evaluate_all_answers(request.qa_pairs)
        return AllEvaluateResponse(overall_feedback=feedback)
...
async def evaluate_portfolio(request: PortfolioEvaluateRequest):
    try:
        feedback = await llm_processor.evaluate_portfolio(request.html_content)
        return PortfolioEvaluateResponse(portfolio_feedback=feedback)
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
